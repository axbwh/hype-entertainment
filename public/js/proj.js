...
let titles = Array.from(document.querySelectorAll(".he-project-title")),
  hovered = false;
...
function init(cvs, scrollWrap, scrollTgt) {
  ...
  titles.forEach((t) => {
    t.addEventListener("mouseenter", () => {
      document
        .querySelector(".he-video-caption.desk")
...
class Project {
  ...
  init() {
    ...
    this.promise =
      this.id === "space"
        ? loadOBJ(`Models/astrofloat.obj`)
        : loadOBJ(`Models/proj/${this.id}.obj`);
...
function render(reset = false) {
  if (reset) {
    ...
    let target = {
      x: map(0.1, 0, 1, camera.position.x, axes.x),
      y: map(0.1, 0, 1, camera.position.y, axes.y),
      z: map(0.1, 0, 1, camera.position.z, axes.z),
    };
    ...
    projects[Math.round(axes.i)].obj.rotation.y = map(
      0.1,
      0,
      1,
      projects[Math.round(axes.i)].obj.rotation.y,
      rotate.y
    );
    projects[Math.round(axes.i)].obj.rotation.x = map(
      0.1,
      0,
      1,
      projects[Math.round(axes.i)].obj.rotation.x,
      rotate.x
    );

    projects[0].zoom = map(0.1, 0, 1, projects[0].zoom, axes.sclZoom);
    projects[0].setScale();
  }
  ...
  ambientLight.intensity = map(0.5, 0, 1, ambientLight.intensity, lightTo);

  bloomPass.strength = map(0.15, 0, 1, bloomPass.strength, bloomTo);
  bloomPass.radius = map(0.15, 0, 1, bloomPass.radius, bloomRadius);
